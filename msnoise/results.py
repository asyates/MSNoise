...
class MSNoiseResult:
    ...
    def plot_distance(
        ...
        """Wiggle distance plot of CCFs or reference stacks sorted by interstation distance.

        Fetches data via :meth:`get_ref` (``source="ref"``) or :meth:`get_ccf`
        (``source="ccf"``), retrieves distances via :meth:`get_distances`, and
        renders a wiggle plot with positive/negative half-cycle fills.

        :param comp:      Component code, e.g. ``"ZZ"``.
        :param mov_stack: Moving-stack tuple required for ``source="ccf"``
                          (e.g. ``("30D", "1D")``).  Ignored for ``source="ref"``.
        :param source:    ``"ref"`` for reference stacks, ``"ccf"`` for moving stacks.
        :param scale:     Wiggle amplitude as fraction of median interstation spacing.
        :param maxlag:    Clip time axis to ±*maxlag* seconds.  ``None`` = full axis.
        :param ax:        Existing :class:`matplotlib.axes.Axes` to plot into.
                          If ``None``, a new figure is created.
        :param color_pos: Fill colour for positive half-cycles.
        :param color_neg: Fill colour for negative half-cycles.
        :param lw:        Line width for waveform traces.
        ...
        # ── fetch data ────────────────────────────────────────────────────
        if source == "ref":
            data = self.get_ref(components=comp)          # {(pair, comp): DataArray}
        elif source == "ccf":
            if mov_stack is None:
                raise ValueError("mov_stack is required for source='ccf'")
            data = self.get_ccf(components=comp, mov_stack=mov_stack)
        else:
            raise ValueError(f"source must be 'ref' or 'ccf', got {source!r}")

        if not data:
            raise ValueError(f"No data found for comp={comp!r}")

        # ── distances ─────────────────────────────────────────────────────
        pairs_in_data = list({pair for pair, _ in data.keys()})
        dists = self.get_distances(pairs=pairs_in_data)

        # ── build sorted rows ─────────────────────────────────────────────
        taxis = list(data.values())[0].coords["taxis"].values
        ...
        rows = []
        for (pair, c), da in data.items():
            if c != comp:
                continue
            if pair not in dists:
                continue
            trace = da.values[mask] if maxlag is not None else da.values
            rows.append((dists[pair], pair, trace))
        rows.sort(key=lambda x: x[0])

        if not rows:
            raise ValueError("No pairs with known distances — check project.yaml has stations.")

        dist_values = [d for d, _, _ in rows]
        span = max(dist_values) - min(dist_values)
        dy = (span / len(rows)) if len(rows) > 1 else 1.0
        ...
        for dist, pair, trace in rows:
            norm = np.nanmax(np.abs(trace))
            if norm == 0:
                continue
            tr = trace / norm * dy * scale
            ax.plot(taxis, tr + dist, color="k", lw=lw, alpha=0.8)
            ax.fill_between(taxis, dist, tr + dist,
                            where=tr > 0, color=color_pos, alpha=0.6,
                            interpolate=True)
            ax.fill_between(taxis, dist, tr + dist,
                            where=tr < 0, color=color_neg, alpha=0.4,
                            interpolate=True)

        ax.set_xlabel("Lag (s)")
        ax.set_ylabel("Distance (km)")
        title = f"{'Ref stacks' if source == 'ref' else f'CCF {mov_stack}'} — {comp}"
        ax.set_title(title)
